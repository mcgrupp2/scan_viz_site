...
slider = alt.binding_range(min=0, max=1, step=0.001)
select_thresh = alt.selection_single(name="High", fields=['thresh'],
                                   bind=slider, init={'thresh': round(eclf_best_thresh,3)})

slider2 = alt.binding_range(min=0, max=1, step=0.001)
select_thresh2 = alt.selection_single(name="Low", fields=['thresh'],
...
genre_dropdown = alt.binding_select(options=np.unique(source_df['classifier']))
genre_select = alt.selection_single(fields=['classifier'], bind=genre_dropdown, name="Genre",init={'classifier': 'et'})
...
).properties(title="Model Diagnosed",width=600).interactive()
...
).properties(title="Higher Performance Tests",width=600).interactive()
...
).properties(title="Rapid Diagnostic Tests",width=600).interactive()
...
bar2=alt.Chart(source_df).transform_filter(
    ...
    y=alt.Y('pct:Q',scale=alt.Scale(domain=(0, 1)),title="Percentage of Positives"),
    tooltip=[alt.Tooltip('pct:Q',format='.3p')]
...
ind_chart=(base+yrule2|((bar+text)|(bar2+text2)))&(base2+yrule+yrule2|((bar3+text3)|(bar4+text4)))&(base3+yrule|((bar5+text5)|(bar6+text6)))
